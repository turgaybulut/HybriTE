...
OXFORD_BLUE = "#002147"
OXFORD_LIGHT_BLUE = "#00509E"
OXFORD_VERMILION = "#D55E00"
OXFORD_TEAL = "#009E73"
OXFORD_PURPLE = "#7B5AA6"
OXFORD_ORANGE = "#E69F00"
OXFORD_SKY = "#56B4E9"
OXFORD_GREY = "#666666"

MODEL_COLORS = {
    "hybrite": OXFORD_LIGHT_BLUE,
    "ribonn": OXFORD_VERMILION,
    "saluki": OXFORD_TEAL,
}
...
ABLATION_COLORS = {
    "hybrite": OXFORD_LIGHT_BLUE,
    "hybrite_nobio": OXFORD_ORANGE,
    "hybrite_nornaplfold": OXFORD_PURPLE,
    "hybrite_nornaplfold_nobio": OXFORD_GREY,
}
...
def figure_performance_analysis() -> None:
    ...
    for i, species in enumerate(["human", "mouse"]):
        ...
        cols_bar = [OXFORD_TEAL] * 5 + [OXFORD_VERMILION] * 5
...
def figure_shap_analysis() -> None:
    ...
    BIO_CATS = {
        "encori": (OXFORD_LIGHT_BLUE, "ENCORI"),
        "clip": (OXFORD_TEAL, "eCLIP"),
        "m6a": (OXFORD_VERMILION, "m6A"),
        "mirna": (OXFORD_PURPLE, "miRNA"),
        "other": (OXFORD_GREY, "Other"),
    }
    ...
    for i, species in enumerate(species_list):
        ...
        try:
            ...
            for k in range(len(node_imp)):
                if k < 7:
                    cols.append(OXFORD_GREY)
                elif k < 13:
                    cols.append(OXFORD_SKY)
                elif k < 19:
                    cols.append(OXFORD_ORANGE)
                else:
                    cols.append(OXFORD_VERMILION)
            ...
            feat_legs = [
                patches.Patch(color=OXFORD_GREY, label="Base"),
                patches.Patch(color=OXFORD_SKY, label="5'UTR"),
                patches.Patch(color=OXFORD_ORANGE, label="CDS"),
                patches.Patch(color=OXFORD_VERMILION, label="3'UTR"),
            ]
            ax_node.legend(
                handles=feat_legs,
                loc="upper right",
    ...
    for i, species in enumerate(species_list):
        ...
        try:
            ...
            ax_reg.axvspan(-0.5, 7.5, color=OXFORD_SKY, alpha=0.2, label="5'UTR")
            ax_reg.axvspan(7.5, 39.5, color=OXFORD_ORANGE, alpha=0.2, label="CDS")
            ax_reg.axvspan(39.5, 55.5, color=OXFORD_VERMILION, alpha=0.2, label="3'UTR")
        ...
        try:
            ...
            type_map = {
                "sequential": ("o", OXFORD_BLUE, "Seq"),
                "structural": ("^", OXFORD_TEAL, "Struct"),
                "long_range": ("*", OXFORD_GREY, "Long-R"),
            }

            ax_edge.axvspan(-0.5, 7.5, color=OXFORD_SKY, alpha=0.15)
            ax_edge.axhspan(-0.5, 7.5, color=OXFORD_SKY, alpha=0.15)
            ax_edge.axvspan(7.5, 39.5, color=OXFORD_ORANGE, alpha=0.15)
            ax_edge.axhspan(7.5, 39.5, color=OXFORD_ORANGE, alpha=0.15)
            ax_edge.axvspan(39.5, 55.5, color=OXFORD_VERMILION, alpha=0.15)
            ax_edge.axhspan(39.5, 55.5, color=OXFORD_VERMILION, alpha=0.15)
...
def _create_per_target_lollipop(
    ...
    for x_grid in grid_lines:
        ax.axvline(x_grid, color=OXFORD_GREY, lw=0.3, alpha=0.2, zorder=0)
